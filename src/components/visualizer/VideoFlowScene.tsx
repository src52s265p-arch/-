...
const fragmentShaderSource = `
  ...
  float glowLine(float distanceToLine, float width, float softness) {
    float core = exp(-distanceToLine * distanceToLine / max(width * width, 0.00001));
    float halo = exp(-distanceToLine * distanceToLine / max(softness * softness, 0.00001));
    return core * 0.72 + halo * 0.22;
  }

  vec3 preserveHighlightDetail(vec3 color) {
    color = max(color, vec3(0.0));
    color *= 1.34;
    float maxChannel = max(max(color.r, color.g), color.b);
    float knee = 0.78;
    if (maxChannel > knee) {
      float over = maxChannel - knee;
      float compressedMax = min(0.98, knee + over / (1.0 + over * 2.45));
      color *= compressedMax / maxChannel;
    }
    color = pow(color, vec3(0.8));
    float lum = dot(color, vec3(0.2126, 0.7152, 0.0722));
    color = mix(vec3(lum), color, 1.28);
    return clamp(color, vec3(0.0), vec3(0.98));
  }
  ...
  void main() {
    ...
    float n_x = noise(flow_coord * 1.7 + warp2 + eps.xy) - n_val;
    float n_y = noise(flow_coord * 1.7 + warp2 + eps.yx) - n_val;
    ...
    float spec_highlight = pow(max(0.0, dot(reflection, view_direction)), 30.0) * active_river;

    float sparkle_pattern = pow(hash(floor(flow_coord * 15.0)), 18.0) * hash(floor(flow_coord * 9.0 + vec2(10.0)));
    float glittering = sparkle_pattern * (0.12 + u_treble * 0.72) * active_river;
    ...
    for (int i = 0; i < 3; i++) {
      ...
      particle_layer += drawParticle(uv, offset_p, p_size) * (0.28 + u_treble * 0.34);
    }

    vec2 ribbon_uv = uv;
    ribbon_uv.x += sin(uv.y * 2.2 + u_time * 0.26) * (0.05 + u_bass * 0.045);
    ribbon_uv.y += cos(uv.x * 1.7 - u_time * 0.22) * (0.03 + u_mid * 0.035);

    float ribbon_layer = 0.0;
    float ribbon_edge = 0.0;
    vec3 ribbon_color = vec3(0.0);
    for (int i = 0; i < 5; i++) {
      float fi = float(i);
      float lane = (fi - 2.0) * 0.28;
      float phase = u_time * (0.56 + fi * 0.045 + u_tunnel_speed * 0.12) + fi * 1.37 + u_bass_phase * 0.08;
      float curve = sin(ribbon_uv.x * (1.16 + fi * 0.18) + phase) * (0.13 + u_mid * 0.055);
      curve += sin(ribbon_uv.x * (3.4 + fi * 0.11) - phase * 1.18) * (0.025 + u_treble * 0.028);
      float distanceToLine = abs(ribbon_uv.y - lane - curve);
      float width = 0.011 + u_bass * 0.012 + sin(phase) * 0.003;
      float stripe = glowLine(distanceToLine, width, 0.075 + u_treble * 0.036);
      float dash = smoothstep(0.12, 0.82, noise(vec2(ribbon_uv.x * 3.5 - u_time * 0.7, fi * 3.1)));
      float laneFade = smoothstep(-1.75, -0.15, ribbon_uv.x) * (1.0 - smoothstep(0.35, 1.85, ribbon_uv.x));
      float pulse = 0.55 + 0.45 * sin(u_time * (1.1 + fi * 0.08) + fi + u_treble_phase * 0.11);
      float intensity = stripe * laneFade * (0.56 + dash * 0.5) * (0.78 + pulse * 0.2 + u_bass * 0.1);
      vec3 laneColor = mix(u_primary_color, u_accent_color, fract(fi * 0.37 + u_time * 0.035));
      laneColor = mix(laneColor, u_secondary_color, 0.24 + 0.18 * sin(phase));
      ribbon_layer += intensity;
      ribbon_edge += smoothstep(width * 5.5, width, distanceToLine) * laneFade;
      ribbon_color += laneColor * intensity;
    }

    float tunnel = 0.0;
    for (int i = 0; i < 5; i++) {
      float fi = float(i);
      float ring = fract(radius_polar * (2.15 + fi * 0.42) - u_time * (0.075 + u_tunnel_speed * 0.055) - fi * 0.11);
      float ringLine = smoothstep(0.08, 0.0, abs(ring - 0.5));
      tunnel += ringLine * exp(-radius_polar * (0.72 + fi * 0.08)) * (0.12 + u_bass * 0.035);
    }

    float center_flash = exp(-length(uv * vec2(1.0, 1.45)) * (2.35 - u_bass * 0.16)) * (0.08 + u_bass * 0.055 + u_treble * 0.025);

    vec3 neon_primary = u_primary_color * n_val * (1.28 + u_bass * 0.12);
    vec3 neon_secondary = u_secondary_color * (warp1.x * 0.6 + warp2.y * 0.4) * (1.18 + u_mid * 0.1);
    vec3 neon_accent = u_accent_color * warp1.y * 0.92;
    vec3 liquid_stream_color = mix(vec3(0.035, 0.018, 0.08) * (1.0 + u_bass * 0.12), neon_secondary, active_river * 0.95);
    liquid_stream_color += neon_primary * active_river * 0.82;
    liquid_stream_color += neon_accent * active_river * 0.58;

    vec3 silver_white = vec3(0.58, 0.9, 1.0) * spec_highlight * 0.46;
    vec3 golden = vec3(0.7, 0.42, 1.0) * glittering * u_sparkle_brightness * 0.72;
    vec3 color_out = liquid_stream_color + silver_white + golden + (u_accent_color * particle_layer * u_sparkle_brightness * 0.24);
    color_out += ribbon_color * (0.78 + u_bass * 0.1);
    color_out += mix(u_secondary_color, u_accent_color, 0.55) * ribbon_edge * 0.18;
    color_out += mix(u_primary_color, u_accent_color, 0.66) * tunnel * 1.75;
    color_out += vec3(0.42, 0.86, 1.0) * center_flash * 1.7;

    float vignette = 1.0 - smoothstep(0.4, 1.8, length(uv));
    color_out *= vignette;
    color_out *= u_brightness * 1.26;

    float random_grain = (hash(gl_FragCoord.xy * 0.03 + vec2(fract(u_time))) - 0.5) * 0.045;
    color_out += random_grain * (0.18 + u_treble * 0.28);

    gl_FragColor = vec4(preserveHighlightDetail(color_out), 1.0);
  }
`;
...
const softLimit = (value: number, knee = 0.72, ceiling = 1.12) => {
  if (value <= knee) return value;
  const over = value - knee;
  return Math.min(ceiling, knee + (1 - Math.exp(-over * 2.2)) * (ceiling - knee));
};
...
const compileShader = (gl: WebGLRenderingContext, source: string, type: number) => {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error('Video Flow shader compilation error:', gl.getShaderInfoLog(shader));
...
export function VideoFlowScene() {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      setShaderError('WebGL is not available for the Video Flow visual.');
      return undefined;
    }
    ...
    if (!vertexShader || !fragmentShader) {
      setShaderError('Video Flow shader failed to compile.');
      return undefined;
    }
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      setShaderError(`Video Flow shader failed to link: ${gl.getProgramInfoLog(program)}`);
      return undefined;
    }
    ...
    const resize = () => {
      ...
      const dprLimit = isScreenOutput ? 1.15 : 1.0;
    ...
    const render = (timestamp: number) => {
      ...
      const energyDrive = 0.86 + liveControls.energyY * 0.48;
      const rhythmTightness = 0.72 + liveControls.rhythmX * 0.72;
      const punch = 0.72 + liveControls.rhythmY * 1.12;
      const bass = softLimit(Math.max(audio.bass, audio.subBass) * energyDrive * live, 0.66, 1.08);
      const mid = softLimit(Math.max(audio.mid, audio.lowMid) * (0.78 + liveControls.textureX * 0.3) * live, 0.68, 1.04);
      const treble = softLimit(Math.max(audio.treble, audio.highMid) * (0.8 + liveControls.textureY * 0.32 + liveControls.atmosphereX * 0.12) * live, 0.64, 1.06);
      ...
      const coolPrimary = blendRgb(hexToRgb(store.baseColor || '#38d9ff'), [0.18, 0.78, 1.0], 0.58);
      const coolSecondary = blendRgb(hexToRgb(store.secondaryColor || '#12123a'), [0.035, 0.035, 0.16], 0.64);
      const coolAccent = blendRgb(hexToRgb(store.accentColor || '#b8f4ff'), [0.48, 0.86, 1.0], 0.5);
      const warmMix = clamp01(liveControls.colorX) * 0.24;
      const colorLift = 0.9 + liveControls.colorY * 0.2;
      ...
      const audioLift = softLimit(audio.energy * 0.55 + audio.beat * 0.32 + audio.transient * 0.2, 0.42, 0.82);
      setFloat('u_brightness', 1.02 + liveControls.colorY * 0.14 + liveControls.atmosphereX * 0.12 + audioLift * 0.12);
      ...
      setFloat('u_sparkle_brightness', 0.72 + liveControls.atmosphereX * 0.42 + liveControls.textureY * 0.24 + treble * 0.28 + softLimit(audio.transient, 0.5, 0.92) * 0.12);
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 block h-full w-full bg-black" aria-label="Video Flow visual template" />;
}
